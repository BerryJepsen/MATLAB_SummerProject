...
switch dct_hide
    %提取方法1
    case 1
        msg_bit = bitget(C_rr,1,'int64');
        msg_bit = msg_bit(:);
        msg_res = bin2msg(num2str(msg_bit));
        disp(['变换域隐藏方法1: ',msg_res]);
    %提取方法2
    case 2
        hide_th = 39;
        QTAB_zigzag = Zigzag(QTAB);
        hide_indice = find(QTAB_zigzag<=hide_th);
        target_C = C_rr(hide_indice,:);
        msg_bit = bitget(target_C,1,'int64'); 
        msg_bit = msg_bit(:);
        msg_res = bin2msg(num2str(msg_bit));
        disp(['变换域隐藏方法2: ',msg_res]);
    %提取方法3
    case 3
        msg_bit = zeros(size(C_rr,2),1);
        for i=1:size(C_rr,2)
           nonzero_idx = find(C_rr(:,i));
           msg_bit(i)=C_rr(nonzero_idx(end),i);
        end
        msg_bit = (msg_bit+1)/2;
        msg_bit = msg_bit(:);
        msg_res = bin2msg(num2str(msg_bit));
        disp(['变换域隐藏方法3: ',msg_res]);
end
